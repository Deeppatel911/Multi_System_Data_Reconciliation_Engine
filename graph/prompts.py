...
It is standard business practice for CRM systems to use a general contact (e.g., admin@...) and Billing systems to use a finance contact (e.g., finance@...).
If a geographical domain mismatch or entity split has already been flagged under `domain`, do NOT list `billing_email` as a separate independent discrepancy just because its domain extension reflects that same split. Consolidate overlapping domain and email differences into a single cohesive domain discrepancy report to avoid over-segmenting.
